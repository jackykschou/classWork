        ...
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	for(char c: value.toCharArray())
            		if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
            			mr.collect_map(Character.toString(Character.toLowerCase(c)), list("1"));
            }
            };
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map4 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                int sLength = grepkey.length();
                char valueArray[] = value.toCharArray();
                Cons result = cons(null, null);
            	for(int i = 0; i < valueArray.length - sLength; i++)
            		if(value.substring(i, i + sLength).equals(grepkey))
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	Cons result = null;
            	for ( Cons lst = value; lst != null; lst = rest(lst) )
                    result = cons(first((Cons)first(lst)), result);
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
            	...
            	mr.collect_map(number, list(rating));
            } };
        ...
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
            	...
            	int sum = 0;
            	double count = 0;
            	for ( Cons lst = value; lst != null; lst = rest(lst),count++ )
                    sum += Integer.decode((String) first((Cons)
                                                         first(lst)));
                mr.collect_reduce(key, sum/count); 
            } };
