...
CDS *Read_CDS_file_entry(FILE *CDS_file)
{
    ...
    //modified (calcuation of new fields)
    cds->number_of_low_order_bits = which_power(cds->cache_line_size);
    cds->low_order_mask = mask_of(cds->number_of_low_order_bits);
    cds->number_of_sets = cds->number_of_cache_entries/cds->number_of_ways;
    cds->sets_bits_mask = mask_of(which_power(cds->number_of_sets));
    if(cds->number_of_ways == 1)
    {
        cds->type = DIRECT;
    }
    else if(cds->number_of_ways == cds->number_of_cache_entries)
    {
        cds->type = FULL;
    }
    else{cds->type = OTHER;}
